...
SFVec2f::SFVec2f(const float value[]) 
{
	setType(fieldTypeSFVec2f);
	setValue(value);
	InitializeJavaIDs();
...
void SFVec2f::getValue(float value[]) const
{
	value[0] = mValue[0];
	value[1] = mValue[1];
}

const float *SFVec2f::getValue() const
{
	return mValue;
}
...
void SFVec2f::setValue(const float value[]) 
{
	mValue[0] = value[0];
	mValue[1] = value[1];
}
...
void SFVec2f::add(const float value[]) 
{
	mValue[0] += value[0];
	mValue[1] += value[1];
}
...
void SFVec2f::translate(const float value[]) 
{
	add(value);
}
...
void SFVec2f::sub(const float value[]) 
{
	mValue[0] -= value[0];
	mValue[1] -= value[1];
}
...
void SFVec2f::scale(const float value[2]) 
{
...
std::ostream& MyVrml::operator<<(std::ostream &s, const SFVec2f &vector) 
{
...
const char *SFVec2f::getValue(char *buffer, int bufferLen) const
{
...
bool SFVec2f::equals(Field *field) const
{
...
bool SFVec2f::equals(const float value[2]) const
{
	SFVec2f vector(value);
	return equals(&vector);
}

bool SFVec2f::equals(float x, float y) const
{
	SFVec2f vector(x, y);
	return equals(&vector);
}
