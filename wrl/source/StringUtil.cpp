...
std::ostream& MyVrml::operator<<(std::ostream &s, const String &value) 
{
...
bool MyVrml::HasString(const char *value)
{
...
int MyVrml::StringLength(const char *value)
{
	if (value == NULL)
		return 0;
	return strlen(value);
...
void String::appendu(const char *value)
{
	...
	else
	{
		...
		int len = nmValLen + 1;
		char *pcTemp = new char[len];
		strcpy(pcTemp, mValue);
		delete[] mValue;
		len = nmValLen + nValLen + 1;
		mValue = new char[len];
