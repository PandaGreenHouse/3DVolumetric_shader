...
std::ostream& MyVrml::operator<<(std::ostream &s, const SFInt32 &value) 
{
...
const char *SFInt32::getValue(char *buffer, int bufferLen) const
{
	_snprintf(buffer, bufferLen, "%d", getValue());
	return buffer;
}

const char *SFInt32::getValueu(char *buffer, int bufferLen) const
{
  sprintf(buffer, "%u", getValue());
  return buffer;
}
...
bool SFInt32::equals(Field *field) const
{
