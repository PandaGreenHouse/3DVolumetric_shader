...
void SFRotation::getValue(float value[]) const
{
	mVector.getValue(value);
	value[3] = mAngle;
}

void SFRotation::getVector(float vector[]) const
{
...
std::ostream& MyVrml::operator<<(std::ostream &s, const SFRotation &rotation) 
{
...
const char *SFRotation::getValue(char *buffer, int bufferLen) const
{
	snprintf(buffer, bufferLen, "%g %g %g %g", getX(), getY(), getZ(), getAngle());
	return buffer;
}
...
bool SFRotation::equals(Field *field) const
{
...
bool SFRotation::equals(float value[4]) const
{
	SFRotation rotation(value);
	return equals(&rotation);
}

bool SFRotation::equals(float x, float y, float z, float angle) const
{
...
void SFRotation::getSFMatrix(SFMatrix *matrix) const
{
	float vector[3];
	getVector(vector);
