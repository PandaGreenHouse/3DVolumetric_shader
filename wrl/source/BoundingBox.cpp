...
*		- Fixed addBoundingBox() to return when the size is invalid.
*		- Added a SFMatrix parameter to addBoundingBox().
*
******************************************************************/
...
void BoundingBox::addBoundingBox(SFMatrix *mx, float center[3], float size[3])
{
...
void BoundingBox::addBoundingBox(SFMatrix *mx, BoundingBox *bbox)
{
	float	center[3];
	float	size[3];
	bbox->getCenter(center);
	bbox->getSize(size);
	addBoundingBox(mx, center, size);
}
...
int BoundingBox::getNPoints() const
{
	return mNPoints;
}
...
void BoundingBox::getMinPosition(float pos[3]) const
{
...
void BoundingBox::getMaxPosition(float pos[3]) const
{
...
void BoundingBox::getSize(float size[3]) const
{
	if (0 < getNPoints()) {
