...
void SFMatrix::getValue(float value[4][4]) const
{
	for (int i=0; i<4; i++) {
		for (int j=0; j<4; j++)
...
void SFMatrix::getTranslation(float value[]) const
{
	value[0] = mValue[3][0];
	value[1] = mValue[3][1];
	value[2] = mValue[3][2];
...
const char *SFMatrix::getValue(char *buffer, int bufferLen) const
{
	buffer[0] = '\0';
	return buffer;
}
...
void SFMatrix::getSFRotation(SFRotation *rotation) const
{
	float x, y, z, w;
	float m[4][4];
	getValue(m);
...
float SFMatrix::determinant() const
{
