...
SFVec3f::SFVec3f(const float value[]) 
{
	setType(fieldTypeSFVec3f);
	setValue(value);
	InitializeJavaIDs();
...
void SFVec3f::getValue(float value[]) const
{
	value[0] = mValue[0];
	value[1] = mValue[1];
	value[2] = mValue[2];
...
const float *SFVec3f::getValue() const
{
	return mValue;
}
...
void SFVec3f::setValue(const float value[]) 
{
	mValue[0] = value[0];
	mValue[1] = value[1];
	mValue[2] = value[2];
...
void SFVec3f::add(const float value[]) 
{
	mValue[0] += value[0];
	mValue[1] += value[1];
	mValue[2] += value[2];
...
void SFVec3f::translate(const float value[]) 
{
	add(value);
}
...
void SFVec3f::sub(const float value[]) 
{
	mValue[0] -= value[0];
	mValue[1] -= value[1];
	mValue[2] -= value[2];
...
void SFVec3f::scale(const float value[3]) 
{
...
void SFVec3f::rotate(const float value[3]) 
{
...
const char *SFVec3f::getValue(char *buffer, int bufferLen) const
{
...
bool SFVec3f::equals(Field *field) const
{
...
bool SFVec3f::equals(const float value[3]) const
{
	SFVec3f vector(value);
	return equals(&vector);
}

bool SFVec3f::equals(float x, float y, float z) const
{
...
std::ostream& MyVrml::operator<<(std::ostream &s, const SFVec3f &vector) 
{
