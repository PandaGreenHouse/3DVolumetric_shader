...
SceneGraph::~SceneGraph() 
{
	...
	delete mBackgroundNodeVector;		mBackgroundNodeVector = NULL;
	delete mFogNodeVector;				mFogNodeVector = NULL;
	delete mNavigationInfoNodeVector;	mNavigationInfoNodeVector = NULL;
	delete mViewpointNodeVector;		mViewpointNodeVector = NULL;

	delete mDefaultBackgroundNode;		mDefaultBackgroundNode = NULL;
	delete mDefaultFogNode;				mDefaultFogNode = NULL;
	delete mDefaultNavigationInfoNode;	mDefaultNavigationInfoNode = NULL;
	delete mDefaultViewpointNode;		mDefaultViewpointNode = NULL;

#if defined(MYVRML_SUPPORT_URL)
	delete mUrl;	mUrl = NULL;
#endif
...
bool SceneGraph::loadA(const char *filename, bool bInitialize, void (*callbackFn)(int nLine, void *info), void *callbackFnInfo)
{
	clear();
	return addA(filename, bInitialize, callbackFn, callbackFnInfo);
}

bool SceneGraph::addA(const char *filename, bool bInitialize, void (*callbackFn)(int nLine, void *info), void *callbackFnInfo)
{
	int fileFormat = GetFileFormatA(filename);
	...
	bool parserRet = parser->loadA(filename, callbackFn, callbackFnInfo);
...
bool SceneGraph::loadW(const wchar_t *filename, bool bInitialize, void (*callbackFn)(int nLine, void *info), void *callbackFnInfo)
{
	clear();
	return addW(filename, bInitialize, callbackFn, callbackFnInfo);
}

bool SceneGraph::addW(const wchar_t *filename, bool bInitialize, void (*callbackFn)(int nLine, void *info), void *callbackFnInfo)
{
	int fileFormat = GetFileFormatW(filename);
	...
	bool parserRet = parser->loadW(filename, callbackFn, callbackFnInfo);
	presult->setParserResult(parserRet);
	if (parserRet == false) {
		delete parser;
		return false;
