...
void SFColor::getValue(float value[]) const
{
	value[0] = mValue[0];
	value[1] = mValue[1];
	value[2] = mValue[2];
...
const float *SFColor::getValue() const
{
	return mValue;
}

float SFColor::getRed() const
{
	return mValue[0];
}

float SFColor::getGreen() const
{
	return mValue[1];
}

float SFColor::getBlue() const
{
	return mValue[2];
}
...
void SFColor::add(const float value[]) 
{
...
void SFColor::sub(const float value[]) 
{
...
std::ostream& MyVrml::operator<<(std::ostream &s, const SFColor &vector) 
{
...
const char *SFColor::getValue(char *buffer, int bufferLen) const
{
	snprintf(buffer, bufferLen, "%g %g %g", getRed(), getGreen(), getBlue());
	return buffer;
}
...
bool SFColor::equals(Field *field) const
{
