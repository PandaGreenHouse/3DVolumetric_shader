...
UrlFile::~UrlFile()
{
	...
	delete	mUrlString;	mUrlString = NULL;
}
...
void UrlFile::setUrl(const char *urlString)	
{
...
const char *UrlFile::getUrl() const
{
...
bool UrlFile::getStream(const char *urlString)
{
	JNIEnv		*jniEnv	= GetJniEnv();
	jboolean	result = 0;
	...
	if (urlString && strlen(urlString)) {
		...
		if (!result) {
			...
			if (url && strlen(url)) {
				int len =strlen(url) + strlen(urlString) + 1;
				char *newUrlString = new char [len];
...
const char *UrlFile::getOutputFilename() const
{
